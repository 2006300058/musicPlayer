<template>
    <div class="flex song-item items-center w-full hover-bg-main" :class="{'playing':id===song.id}"
      ...
      <div class="flex-shrink-0 flex-1 flex items-center justify-between pr-5 ">
        ...
        <div class="items-center flex flex-1 w-10 flex-shrink-0">
          ...
          <IconPark v-if="song.mv>0" class="ml-2 text-orange-400 cursor-pointer" size="18"
      ...
      <div class="flex-shrink-0" :class="{'w-1/4':showAlName,'w-1/3':!showAlName}" v-if="showArName">
        <div class="w-9/12 truncate">
          <small class="truncate max-w-full hover-text text-sm"
      ...
      <div class="flex-shrink-0" :class="{'w-1/4':showArName,'w-1/3':!showArName}" v-if="showAlName">
        <div class="w-9/12 truncate">
          <small class="truncate hover-text text-sm" @click="router.push({name:'album',query:{id:song.al.id}})">{{
              song.al.name
            }}</small>
        </div>
      </div>
      <div class="w-20 flex-shrink-0  ">
        <div class="w-20 truncate">
          <small class="text-sm">{{ useFormatDuring(song.dt / 1000) }}</small>
        </div>
      </div>
    </div>
  </template>
  ...
  import {Add, DownTwo, Like, MoreTwo, PlayOne, PlayTwo,PauseOne} from '@icon-park/vue-next'
  ...
  const {play,togglePlay,setPause} = usePlayerStore()
